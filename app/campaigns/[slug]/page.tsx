...
export default function CampaignDetail({ params }: { params: { slug: string } }) {
  const campaign = getCampaignBySlug(params.slug) || (params.slug === "nuova-campagna" ? { ...campaigns[0], slug: "nuova-campagna", name: "Nuova campagna", status: "Bozza" as const, updated: "2026-07-16" } : undefined);
  if (!campaign) return <EmptyState title="Campagna non trovata" description="Lo slug richiesto non corrisponde a nessuna campagna mock. Torna all’archivio per aprire una campagna disponibile." action={<Button asChild><Link href="/campaigns">Vai alle campagne</Link></Button>} />;
